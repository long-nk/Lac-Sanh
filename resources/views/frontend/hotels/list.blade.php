...
@section('title', 'Tìm kiếm ' . @$title . ' tại ' . @$location->name . ' | Viva Trip')
@section('description', 'Danh sách ' . @$title . ' hàng đầu tại ' . @$location->name . ' | Viva Trip')
...
@section('contents')
    ...
    <main>
        ...
        {{--        @else--}}
        <section
            class="site-filter-top sticky-top-custom {{(Route::is('hotels.list_location') || Route::is('hotels.search')) ? 'site-filter-top-search' :''}} {{$agent->isMobile() ? '' : 'sticky-top'}}  sticky-filter">
            <div class="container">

                <div class="add-div-filter">
                    @include('frontend.template.filter')
                    @if($agent->isMobile())
                        @if(Route::is('hotels.list_location') || Route::is('hotels.search'))
                            <div class="js-addivfilter"></div>
                        @endif
                    @endif
                </div>
                @if(Route::is('hotels.list_location') || Route::is('hotels.search'))
                    <div class="js-show-filter">
                            <?php echo svg('slide') ?>
                        <span>Bộ lọc</span>
                    </div>
                    <div class="js-show-arrange">
                            <?php echo svg('short') ?>
                        <span>Sắp xếp</span>
                    </div>
                @else
        ...
        <section class="site-search pd-main">
            @include('frontend.template.filter_bar')
        </section>

        <div class="js-filter-fixed">
            <div class="js-close-filter-vila"><?php echo svg('close') ?></div>
            <div
                ...
                <ul class="nav-filter nav">
                    <?php $listTitle = array('', 'Khách sạn', 'Villa', 'Homestay', 'Resort', 'Du thuyền') ?>
                    ...
                    <li class="nav-item">
                        <a class="nav-link type-hotel {{$value[$i] == intval(@$dataSearch['type']) ? 'active' : ''}}"
                           ...
                           title="<?php echo $listTitle[$i] ?>"><?php echo svg($svg[$i]) ?>
                            <span><?php echo $listTitle[$i] ?></span>
                        </a>
                    </li>
                    <?php } ?>
                    <li>
                        <span class="khac d-flex align-items-center" data-bs-toggle="modal"
                              data-bs-target="#modal-khac">
                            <?php echo svg('dot') ?>
                            Khác
                        </span>
                    </li>
                ...
                <div class="tab-content">
                    <div class="tab-pane fade active show">
                        <form class="form-filter" id="form-search-hotel" action="{{route('hotels.search')}}"
                              method="GET">
                            <input type="text" class="type-hotel" name="type_hotel" id="type-hotel-value"
                                   value="{{intval(@$dataSearch['type']) ?? \App\Models\Comforts::KS}}" hidden="true">
                            <ul>
                                <li class="items js-ganeral">
                                    <span class="items--title">Điểm đến</span>
                                    <input class="items--search js-show-address" type="text" name="location"
                                           id="search-location"
                                    ...
                                    <div class="sub-filter js-sub-custom">
                                        <div class="input-location" style="display: none">
                                            <div class="input-name">
                                                <input class="items--search js-show-address" type="text"
                                                       name="location-mobile" id="search-location-mobile"
                                                       value="{{@$dataSearch['location']}}"
                                                       placeholder="Thành phố, khách sạn, địa điểm">
                                            </div>
                                        ...
                                        <div class="row">
                                            <div class="col-md-4" id="list-location-search-2">
                                                <div class="sub-filter--search">
                                                    <div class="title">
                                                        <h3>Tìm kiếm gần đây</h3>
                                                        <a class="js-del-history" href="javascript:;">Xóa lịch sử
                                                            tìm kiếm</a>
                                                    </div>
                                                    <div class="js-search-history list">
                                                        ...
                                                        @if(count(@$listLocation) > 0)
                                                            @foreach ($listLocation as $l)
                                                                <div class="items-sub choose-location"
                                                                    ...
                                                                    <span
                                                                        class="ks">{{$l['number'] ?? 0}} <?php echo svg('ks1') ?></span>
                                                                </div>
                                                            @endforeach
                                                        @endif
                                                        ...
                                                        @if(count($listLocation) == 0 && count($listHotel) == 0)
                                                            <div class="items-sub" style="width: 100%;">
                                                                <h3 class="information" style="text-align: center">Không
                                                                    có địa đểm
                                                                    nào!</h3>
                                                            </div>
                                                        @endif
                                    ...
                                    <div class="items--flex">
                                        <div class="items--flex--check ">
                                            ...
                                            <p class="items--flex--check__content"
                                               id="t-startdatepp">{{@$dataSearch['startDate'] ? $dataSearch['startDate'] : 'Chọn ngày đi'}}</p>
                                        </div>
                                        <div class="jss139">
                                            <span id="dayCount"
                                ...
                                <li class="items js-ganeral js-show-bookzoom">
                                    ...
                                    <p class="items--content js-content-bookzoom"><span
                                            class="js-zoom">{{@$dataSearch['room'] ? @$dataSearch['room'] : 1}}</span>
                                        phòng, <span
                                            class="js-peo">{{@$dataSearch['people'] ? @$dataSearch['people'] : 1}}</span>
                                        người lớn, <span
                                            class="js-tre">{{@$dataSearch['child'] ? @$dataSearch['child'] : 0}}</span>
                                        trẻ em</p>
                                    <div class="js-check-input">
                                        <input class="input-zoom" type="number"
                                               value="{{@$dataSearch['room'] ? @$dataSearch['room'] : 1}}" min="1"
                                               name="room">
                                        <input class="input-peo" type="number"
                                               value="{{@$dataSearch['people'] ? @$dataSearch['people'] : 1}}" min="1"
                                               name="people">
                                        <input class="input-tre" type="number"
                                               value="{{@$dataSearch['child'] ? @$dataSearch['child'] : 0}}" min="0"
                                               name="child">
                                    </div>
                                    <div class="sub-filter--checkn js-sub-custom">
                                        ...
                                        <div class="js-close-filter js-close-filter-button"><span class="btn btn-blue">Xác nhận</span>
                                        </div>
                                    </div>
        ...
        @if(@$location['content'])
            <section class="site-content-bottom pd-main">
                <div class="container">
                    <div class="raiting-top">
                        <h3>Đánh giá {{@$title}}, khu nghỉ dưỡng ở {{@$location['name']}}</h3>
                        <div class="d-flex align-items-center">
                            <div class="raiting">
                                <span class="number">{{@$location['rate']}}</span>
                                <p>
                                    @if(@$location['rate'] > 9.5)
                                        Tuyệt vời
                                    @elseif(@$location['rate'] > 9)
                                        Xuất sắc
                                    @elseif(@$location['rate'] > 8)
                                        Tốt
                                    @elseif(@$location['rate'] > 7)
                                        Trung bình
                                    @else
                                        Kém
                                    @endif
                    ...
                    <div class="infor tocContent">
                        <div class="row">
                            <div class="col-md-3">
                                <div class="infor--sidebar sticky-top">
                                    <div class="title-menu">
                                        <span class="title-menu">Thông tin chi tiết</span>
                                    </div>
                                    <div class="toc">
